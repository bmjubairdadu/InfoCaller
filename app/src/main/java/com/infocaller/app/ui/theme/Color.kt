...
val surfaceElevated: Color
    @Composable @ReadOnlyComposable
    get() = if (isDarkScheme(MaterialTheme.colorScheme)) Color(0xFF16171C) else Color.White

val surfaceSunken: Color
    @Composable @ReadOnlyComposable
    get() = if (isDarkScheme(MaterialTheme.colorScheme)) Color(0xFF0B0B0E) else Color(0xFFF1F2F5)

val strokeSoft: Color
    @Composable @ReadOnlyComposable
    get() = if (isDarkScheme(MaterialTheme.colorScheme)) Color.White.copy(alpha = 0.08f) else Color.Black.copy(alpha = 0.07f)

val strokeStrong: Color
    @Composable @ReadOnlyComposable
    get() = if (isDarkScheme(MaterialTheme.colorScheme)) Color.White.copy(alpha = 0.18f) else Color.Black.copy(alpha = 0.14f)

val amberGlow: Color
    @Composable @ReadOnlyComposable
    get() = Primary.copy(alpha = if (isDarkScheme(MaterialTheme.colorScheme)) 0.22f else 0.30f)
