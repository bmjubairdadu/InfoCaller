...
class CallerViewModel(
    ...
    fun searchByIdentifier(identifier: String, type: String) {
        ...
        activeSearchJob = viewModelScope.launch {
            if (type == com.infocaller.app.domain.engine.IdentifierType.PHONE) {
                ...
                try {
                    val fromRegistry = repository.getSharedRegistryCaller(safeId)
                    if (fromRegistry != null && generation == searchGeneration &&
                        (!fromRegistry.displayName.isNullOrBlank() || !fromRegistry.photoUrl.isNullOrBlank())
                    ) {
                        _searchResult.value = SearchUiState.Success(fromRegistry, isLive = false)
                        _scanActive.value = false
                        return@launch
                    }
                } catch (_: Exception) { } catch (_: Error) { }
                            ...
                            when (state) {
                                ...
                                is com.infocaller.app.domain.engine.ScanState.Completed -> {
                                    ...
                                    try {
                                        ...
                                        if (hasAnything) _searchResult.value = SearchUiState.Success(mapped!!, isLive = false, livePartial = state.result)
                                        else {
    ...
    fun setPrimaryPhotoAndSave(identifier: String, url: String, provider: String, displayName: String, onDone: (Boolean) -> Unit = {}) {
        if (identifier.isBlank() || url.isBlank()) {
            onDone(false)
            return
        }
        viewModelScope.launch {
            var ok = false
            try {
                ok = contactEnrichmentService.saveContactFast(identifier, displayName.ifBlank { identifier }, url)
            } catch (_: Exception) { ok = false }
            try {
                setPrimaryPhoto(identifier, url, provider)
                ok = true
            } catch (_: Exception) { }
            try { onDone(ok) } catch (_: Exception) { }
        }
    }
