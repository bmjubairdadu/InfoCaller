...
fun NavGraph(
    ...
    val isCoreOk = PermissionManager.hasPermissions(context, PermissionManager.CONTACTS_PERMISSIONS)
    ...
    Box(modifier = androidx.compose.ui.Modifier.fillMaxSize()) {
        NavHost(
            ...
            enterTransition = {
                slideInHorizontally(tween(Motion.NORMAL, easing = Motion.emphasized)) { it / 8 } +
                    fadeIn(tween(Motion.NORMAL, easing = Motion.decelerate))
            },
            exitTransition = {
                slideOutHorizontally(tween(Motion.FAST, easing = Motion.accelerate)) { -it / 10 } +
                    fadeOut(tween(Motion.FAST))
            },
            popEnterTransition = {
                slideInHorizontally(tween(Motion.NORMAL, easing = Motion.emphasized)) { -it / 10 } +
                    fadeIn(tween(Motion.NORMAL, easing = Motion.decelerate))
            },
            popExitTransition = {
                slideOutHorizontally(tween(Motion.FAST, easing = Motion.accelerate)) { it / 8 } +
                    fadeOut(tween(Motion.FAST))
            }
        ) {
            composable("launcher") {
