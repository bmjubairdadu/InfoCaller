...
class ScanOrchestrator(
    ...
    override fun startScan(identifier: String, priority: ScanPriority, type: String): Flow<ScanState> {
        ...
        if (raced?.job?.isActive == true) {
            val existing = _scanStates.value[scanKey]
            if (existing is ScanState.Completed || existing is ScanState.Error) {
                return flowOf(existing)
            }
            return scanStates.map { it[scanKey] ?: ScanState.Idle }
                .filter { it !is ScanState.Idle || _scanStates.value[scanKey] !is ScanState.Idle }
        }
    ...
    private val pausedScansLock = Any()

    private fun cancelBackgroundScans() {
        synchronized(pausedScansLock) {
    ...
    private fun resumeBackgroundScans() {
        ...
        if (!hasActiveForeground) {
            val toResume: List<String> = synchronized(pausedScansLock) {
