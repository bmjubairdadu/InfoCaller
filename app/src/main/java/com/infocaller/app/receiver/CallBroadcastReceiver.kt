                ...
                CoroutineScope(Dispatchers.IO).launch {
                    try {
                        ...
                        if (resolvedNumber != null) {
                            ...
                            val allDigits = clean.filter { it.isDigit() }
                            val d = if (allDigits.length >= 6) allDigits.takeLast(6) else allDigits
                            if (d.isNotBlank() && isVerificationCall(context, clean)) com.infocaller.app.util.OtpManager.onMissedCallTailSync(d, clean, isIdle = true)
                            if (!isStaleVerificationTail(context, clean)) {
                                identifyMissedCall(context, clean)
                            }
                        }
            ...
            try {
                withTimeoutOrNull(9000) {
                    ...
                    if (online && !known) {
                        ...
                        val found = withTimeoutOrNull(8000) {
                            var result: com.infocaller.app.data.local.entity.ContactEnrichmentEntity? = null
                        ...
                        if (found?.publicName != null) {
                            showMissedCallNotification(context, phoneNumber, found, startedAt, scanned = true)
                        } else {
                            val cached = try {
                                app.database.enrichmentDao().getEnrichmentSync(normalized)
                            } catch (_: Exception) { null }
                            showMissedCallNotification(context, phoneNumber, cached, startedAt, scanned = true)
                        }
                    } else {
                        val cached = try {
                            app.database.enrichmentDao().getEnrichmentSync(normalized)
