...
class TruecallerAuthManager(
    ...
    private fun signingSecret(): String? = try {
        com.infocaller.app.BuildConfig.TRUECALLER_REQUEST_SECRET.trim().takeIf { it.isNotBlank() }
    } catch (_: Exception) { null } catch (_: Error) { null }

    suspend fun requestOtp(phone: String): OtpRequestResult? = withContext(Dispatchers.IO) {
        val secret = signingSecret() ?: return@withContext null
        ...
        val seqNo = (prefs.getInt(sequenceKey, 0) + 1).coerceIn(1, 100)
    ...
    suspend fun verifyOtp(phone: String, requestId: String, otp: String): VerifyResult = withContext(Dispatchers.IO) {
        ...
        val sig = PhoneNumberUtils.getSignificantNumber(norm) ?: norm.filter{it.isDigit()}
        val dial = PhoneNumberUtils.getDialingCode(norm) ?: 880
        val secret = signingSecret() ?: return@withContext VerifyResult(false, null, -1, "Verification not configured")
                ...
                if (txt != null) {
                    ...
                    if (j != null && j.has("status")) {
                        ...
                        if (status == 11 || status == 40101) {
                            val serverMsg = j.get("message")?.takeIf { !it.isJsonNull }?.asString
                                ?.takeIf { it.isNotBlank() } ?: txt.take(300)
                            lastResult = VerifyResult(false, null, status, serverMsg)
                            Log.d("TruecallerAuth", "verifyOtp: status $status on $host — trying next host")
                            continue
                        }
        ...
        for (host in candidateHosts) {
            try {
                ...
                val req = Request.Builder().url("https://$host/v1/completeOnboarding")
                    ...
                    .addHeader("accept-encoding","gzip")
                    .addHeader("user-agent","Truecaller/11.75.5 (Android;10)")
                    .addHeader("clientsecret", signingSecret() ?: return@withContext VerifyResult(false, null, -1, "Verification not configured"))
                    .post(body.toString().toRequestBody("application/json; charset=UTF-8".toMediaType())).build()
