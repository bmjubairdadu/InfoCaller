...
class ContactEnrichmentService(
    ...
    suspend fun updateExistingContact(phoneNumber: String, caller: Caller): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            val rawContactId = resolveWritableRawContactId(contactId)
            ...
            val shouldUpdatePhoto = photoId == -1L && gaps.missingPhoto && caller.photoUrl != null && rawContactId != -1L
            ...
            if (scanEmail != null && rawContactId != -1L && !hasEmailRow(rawContactId, scanEmail)) {
                try {
