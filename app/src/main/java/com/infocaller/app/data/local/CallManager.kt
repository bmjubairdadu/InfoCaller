...
object CallManager {
    ...
    private var activeCallback: Call.Callback? = null

    fun updateCall(call: Call?) {
        val previous = _activeCall.value
        if (previous === call && call != null) return
        activeCallback?.let { cb ->
            try { previous?.unregisterCallback(cb) } catch (_: Exception) { }
        }
        activeCallback = null
        ...
        if (call == null) {
            ...
            val cb = object : Call.Callback() {
                override fun onStateChanged(call: Call, state: Int) {
                    if (_activeCall.value !== call) return
            ...
            activeCallback = cb
            try { call.registerCallback(cb) } catch (_: Exception) { activeCallback = null }
        }
    }
