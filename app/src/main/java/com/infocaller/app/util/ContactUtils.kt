...
object ContactUtils {
    ...
    private val NAME_HONORIFICS = setOf(
        "vai", "bhai", "bhaiya", "bhaia", "vaia", "vaiya", "vaijan", "bhaijan",
        "bro", "brother", "apu", "apa", "sir", "madam", "mam", "maam", "miss",
        "uncle", "aunty", "auntie", "khala", "khalu", "mama", "mami",
        "chacha", "chachi", "kaka", "kaki", "dada", "dadu", "didi",
        "nana", "nani", "boss", "bhabi", "bhabhi", "huzur", "ustad", "ostad"
    )

    private val NAME_LEADING_TITLES = setOf(
        "md", "mohammad", "muhammad", "mohammed", "mohd", "mst", "most", "mosammat",
        "mr", "mrs", "ms", "dr", "engr", "eng", "adv", "advocate",
        "prof", "professor", "alhaj", "alhaz", "hajj", "haji", "late", "sri", "shri"
    )

    fun normalizeMainName(name: String?): String? {
        ...
        val trimmed = name.trim()
            ...
            .substringBefore("/")
            .trim()
        val rawTokens = trimmed.split(Regex("[^\\p{L}\\p{Nd}.]+")).filter { it.isNotBlank() }
        if (rawTokens.isEmpty()) return null
        val kept = mutableListOf<String>()
        for (tok in rawTokens) {
            val low = tok.lowercase().trim('.', ' ')
            if (low.isEmpty()) continue
            if (low in NAME_HONORIFICS) continue
            if (kept.isEmpty() && low in NAME_LEADING_TITLES) continue
            kept.add(tok.trim('.', ' '))
        }
        val result = kept.joinToString(" ").trim()
        if (result.length >= 2 && result.any { it.isLetter() }) return result
        val fallback = rawTokens
            .filter { it.lowercase().trim('.', ' ') !in NAME_HONORIFICS }
            .joinToString(" ").trim()
        return fallback.takeIf { it.length >= 2 && it.any { c -> c.isLetter() } }
    }
