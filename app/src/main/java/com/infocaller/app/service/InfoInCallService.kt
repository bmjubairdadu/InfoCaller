...
class InfoInCallService : InCallService() {
    ...
    private fun showIncomingCallNotification(call: Call, enrichment: com.infocaller.app.data.local.entity.ContactEnrichmentEntity? = null) {
        ...
        serviceScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            try {
                val contactName = if (enrichment?.publicName.isNullOrBlank()) {
                    com.infocaller.app.util.PhoneNumberUtils.getContactName(this@InfoInCallService, number)
                } else null
                val contactPhoto = if (photoUrl == null) {
                    com.infocaller.app.util.PhoneNumberUtils.getContactPhotoUri(this@InfoInCallService, number)
                } else null
                if (contactName == null && contactPhoto == null) return@launch
                val enrichedInfo = com.infocaller.app.data.local.entity.ContactEnrichmentEntity(
                    normalizedPhoneNumber = number,
                    contactId = null,
                    publicName = contactName ?: enrichment?.publicName,
                    about = enrichment?.about,
                    profileImageUrl = contactPhoto ?: enrichment?.profileImageUrl,
                    source = enrichment?.source ?: "contacts",
                    confidence = enrichment?.confidence ?: "0.9",
                    lastChecked = System.currentTimeMillis(),
                    expiresAt = System.currentTimeMillis() + 60_000L
                )
                showIncomingCallNotification(call, enrichedInfo)
            } catch (_: Exception) { }
        }
    }
